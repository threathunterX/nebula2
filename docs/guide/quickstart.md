...
> **当前可运行的是[参考引擎](../../packages/reference-engine/)** —— 一个零依赖的最小实现,用来验证算子语义、跑通策略判定。生产引擎(Flink)仍在开发中,`docker compose` 部署方式见[部署文档](../operations/deployment.md)(🚧)。
>
> 本文能让你在 2 分钟内看到风控系统实际工作的样子:合成攻击流量进去,风险告警出来。
...
参考引擎目前只接受合成场景。要接入真实数据需要等生产引擎。**另外请注意**:本仓库的测试与示例数据严禁包含任何真实流量或个人信息,详见[贡献指南](../../CONTRIBUTING.md)。
