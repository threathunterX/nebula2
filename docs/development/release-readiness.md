...
> | 9 个提交带着本机主机名邮箱 `timbi@Tims-Mac-mini.local` | ✅ 已重写(当时 0 fork / 0 star),`git config` 已修正 |
...
> **仍未处理,且需要仓库所有者出面**:
>
> - **1.x 泄露的凭据尚未轮换**(公网 InfluxDB root、Sentry DSN、Fabric SSH 口令、SMTP
>   账号)。这不在本项目代码范围内,但发布会把注意力引向整个组织。
> - 旧提交仍可通过精确 SHA 从 fork 网络取回,需向 GitHub Support 提交请求。
>
> 组织内另外 7 个 public 仓库(IPSDK 系列、jumpserver、bootcamp fork)已扫描,
> **未发现凭据或个人信息泄露**。
>
> 首次审查报告原文保留在下方。
...
- **邮箱白名单是子串匹配**:`ALLOWED_DOMAINS.search(m.group(0))`,
  所以 `foo@evil-example.com.attacker.net` 会因为含 `example.com` 而通过。
- 公网 IP 正则的组播段排除只写了字面量 `224.`,没覆盖完整的 `224.0.0.0/4`
  (224-239)。次要瑕疵,不构成泄露风险。
