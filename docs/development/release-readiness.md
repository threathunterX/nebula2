...
> ## v0.5.0 发布前的复审(2026-07-27)
>
> 按[发布流程](release-process.md) §3 逐项核对,发现并处理了以下几项:
>
> | 问题 | 处理 |
> |---|---|
> | `docs/guide/quickstart.md` 的测试分布又落后了(参考引擎 139→177、计算引擎 156→196、采集器 4→6 个包) | ✅ 已改为实测值。**连续两次发布都栽在这一处** —— 派生数字没有自动校验,只靠发布前人工核 |
> | `README.md` 写着 `cel-functions` 「🚧 仅有语义文档」 | ✅ 三个函数两个引擎均已实现,标记已去 |
>
> 逐项核对通过的:`make validate` / `test` / `lint` / `privacy-check` / `secrets-scan`
> 全部 exit=0;main 最近一次 CI 全绿;资产数量(事件 17 / 变量 253 / 策略 170 /
> 标签 15)与文档一致;`sanitize_rules` 与 `deploy/compose/.env` 均未被跟踪;
> 提交者邮箱无个人信息;**`docs/README.md` 里已没有整篇占位的文档**,剩余 🚧 全部
> 指向真正未实现的能力(OpenAPI 生成、Kafka 作为数据源、序列的分支模式、存储组件 HA)。
>
> **本版三项主要工作都在真实环境验证过**,不是纸面推演:Kafka 输出对着 Redpanda 数了
> broker 上的实际条数;OpenResty 埋点跑的是官方镜像的真实 OpenResty 1.31.1.1;
> Helm chart 在 k3s 上真装并验证了出厂资产导入。
>
> **一处判断被推翻**:此前多次说「这台机器起不了 k8s」。实际能起 —— Colima 调到
> 6 CPU / 10 GiB 开 k3s,整套空载只用约 1.6 GiB。**受限的是我的估计,不是机器。**
>
> **仍未处理的遗留**(与前几版相同,均非本项目代码可解决):1.x 泄露凭据的轮换,
> 以及向 GitHub Support 提交 fork 网络旧提交的清理请求。
>
> 更早的审查报告原文保留在下方。

> ## v0.4.0 发布前的复审(2026-07-26)
>
> 按[发布流程](release-process.md) §3 逐项核对,发现并处理了以下几项:
>
> | 问题 | 处理 |
> |---|---|
> | **#43(采集器 Prometheus 端点)与 #44(序列检测)漏写 CHANGELOG** —— 两者在 v0.3.0 打 tag 之后合入,`[Unreleased]` 段里却没有它们。0.3.0 段落甚至还留着「采集器的 Prometheus 端点**尚未实现**」 | ✅ 已补入 0.4.0 段落。**流程本身要求「在 PR 中写,不在发布时补写」,这次又是补写的** —— 与 v0.1.0 复审时同一条流程失效 |
> | `docs/guide/quickstart.md` 的测试分布是旧数字(计算引擎 135、控制面 24,且没有管理界面) | ✅ 已改为实测值:参考引擎 139、计算引擎 156、控制面 48、管理界面 9、采集器 4 个包 |
> | 测试态缺陷的**升级后果**没有对使用方讲清楚 | ✅ CHANGELOG 加了「升级须知」小节:内置 170 条全是 `test` 状态,升级后实际生效的拦截会降到零 |
>
> 逐项核对通过的:`make validate` / `test` / `lint` / `privacy-check` / `secrets-scan`
> 全部 exit=0;main 最近一次 CI 全绿;`make demo` 跑通;资产数量(事件 17 / 变量 253 /
> 策略 170 / 标签 15)与文档一致;`sanitize_rules` 与 `deploy/compose/.env` 均未被跟踪;
> 提交者邮箱无个人信息;README 与 `docs/README.md` 的 🚧 标记与实际完成度一致;
> LICENSE 自初始提交起未改动。
>
> **仍未处理的遗留**(与 v0.1.0 时相同,均非本项目代码可解决):1.x 泄露凭据的轮换,
> 以及向 GitHub Support 提交 fork 网络旧提交的清理请求。
>
> 更早的审查报告原文保留在下方。
