...
> **当前状态**:项目尚未发布任何版本,仓库没有任何 tag。本文描述的是首个版本
> 发布时要走的流程,以及此后每次发布的固定动作。首次执行时如发现流程与实际不符,
> 请直接修订本文 —— 一份没人照着做的流程文档比没有流程更糟。
...
星云处理的是业务风控数据,资产直接来自 1.x 的生产配置。**1.x 正是因为在公开仓库中
残留真实客户数据和终端用户凭据造成过实际泄露**,这一节不能走过场。
