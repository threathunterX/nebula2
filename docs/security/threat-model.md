...
| 缺口 | 后果 | 谁来补 |
|---|---|---|
| **采集器 HTTP 入口无认证** | 任何能连到该端口的人都能伪造事件 —— 可以喂假行为把某个账号刷进名单,也可以稀释真实信号 | 部署方:绑定到内网地址,或在前置代理上做 mTLS / 令牌 |
...
系统没做的、且这是**当前最值得关注的缺口**:

- **采集器入口无认证**(见第四节)。伪造事件是投毒最直接的路径,而这条路径当前是开的。
- 没有「同一主体被拉黑的速率」这类元监控。
...
- [ ] **采集器只监听内网地址**,或在前面加认证。这是名单投毒最直接的入口。
