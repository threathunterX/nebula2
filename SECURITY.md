...
- 用户口令使用 **Argon2id**(1.x 为无盐单次 SHA1)。首个管理员账号的随机口令只在首次启动时打印一次
- 服务令牌只存 SHA-256 哈希,明文仅在签发响应中出现一次;可绑定来源网段,令牌与来源 IP 是 **AND** 关系(1.x 使用 5 个硬编码的免登录 token,且鉴权逻辑为「来源 IP 白名单 **或** token 匹配」,拿到 token 即可从任意来源冒充内部身份)
- 人与服务的权限不重叠:管理员不能调 `/checkRisk`,服务令牌不能访问任何管理接口
- 无状态鉴权,不签发会话 cookie(1.x 硬编码 cookie_secret,可离线伪造任意用户会话)
- 未显式放行的路径一律拒绝
