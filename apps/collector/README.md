...
Go 编写,单二进制部署 —— 采集器要装到客户环境里,依赖越少越好。

**直接依赖只有 `franz-go`**(Kafka 输出用),连传递依赖共 5 个模块。除此之外全部用
标准库,包括 syslog 解析与 Prometheus 指标端点。

> 采集器此前是**零外部依赖**的,这条写在多处文档里。引入 Kafka 客户端是明确的项目
> 决定 —— 零依赖是手段不是目的,而「让使用方自己接管道」的代价更高:管道断了采集器
> 不知道,它的指标看不到管道那一头。完整理由见
> [`internal/sink/kafka.go`](internal/sink/kafka.go)。
...
```bash
./nebula-collector -source http -source-addr 127.0.0.1:8088 -events ../../seeds/events \
  -kafka 127.0.0.1:9092 -kafka-topic nebula-events
```

| 参数 | 缺省 | 说明 |
|---|---|---|
| `-kafka` | — | broker 地址,逗号分隔。设了即切到 Kafka 输出 |
| `-kafka-topic` | `nebula-events` | 目标主题 |
| `-kafka-acks` | `all` | `all` / `leader` / `none` |
| `-kafka-compression` | `lz4` | `none` / `gzip` / `snappy` / `lz4` / `zstd` |

**按主体分区**:记录的 key 取 `uid`,没有则 `did`,再没有则 `c_ip`。同一个主体的事件
落到同一分区 —— 下游按 key 分区计算时才不会把同一主体的事件拆到不同并行实例上。

三条要知道的:

- **异步生产。** `Write` 返回成功只表示「已进入发送队列」,不表示已落到 broker。真正的
  投递失败在退出摘要里报,并让进程以非 0 退出码结束。
- **broker 连不上时启动即失败**,而不是等到第一条事件。采集器是常驻进程,
  「起来了但一条也发不出去」是最难察觉的故障形态。
- **`-kafka-acks=none` 会打警告。** 不等确认意味着 broker 拒收时采集器不会知道 ——
  风控链路上少一段数据不会报错,只会让策略不命中。
