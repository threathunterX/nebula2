...
上一版结束时还剩三项挂在那里:Kafka 输出卡在零依赖的取舍上、OpenResty 埋点没有环境
验证、Helm 说这台机器起不了 k8s。这一版三件全部落地,而且**每一件都在真实环境里验证过**,
不是纸面推演。

其中一件是我判断错了:「这台机器起不了 k8s」并不成立 —— Colima 调到 6 CPU / 10 GiB
开 k3s,整套跑起来空载只用约 1.6 GiB。

### 升级须知

**采集器不再是零外部依赖。** 引入 `franz-go` 提供 Kafka 输出(连传递依赖共 5 个模块)。
这是明确的项目决定 —— 零依赖是手段不是目的,而「让使用方自己接管道」的代价更高:
管道断了采集器不知道。README、SECURITY.md 等 7 处相关表述已逐一如实修订。

**CI 编译采集器的 Go 版本从 1.22 提到 1.26**,与发布镜像对齐。此前两者不一致,
也就是说 CI 测的和实际发布的不是同一个 Go。
...
- **OpenResty 埋点**(`apps/collector/openresty/`)。网关侧把 HTTP 请求转成
  ...
  装在 `log_by_lua` 阶段:拿得到状态码,埋点耗时不计入用户延迟,而且**埋点失败不会
  影响业务请求** —— 实测采集器整个挂掉时业务请求仍然 200。

  **敏感字段根本不采。** `cookie` / `uri_query` / `c_body` / `s_body` 采集器本来会脱敏,
  但让数据先出网关再擦掉,中间每一跳都可能留副本。实测:请求带 `Cookie: SECRET_COOKIE`
  与 `?token=SECRET_QUERY`,落盘的 11 条事件里两个值一次都没出现。

  `X-Forwarded-For` **默认不信任** —— 默认信任会让攻击者随便声明自己的 IP。

  用 `ngx.socket.tcp` 手写 POST,**不依赖 lua-resty-http**:那个库不是 OpenResty
  自带的(要 opm 单独装),让部署方为一个埋点脚本再装一个库是实打实的负担。

  用官方镜像的真实 OpenResty 1.31.1.1 验证,不是纸面推演。

- **采集器的 Kafka 输出**(`-kafka`)。按主体分区:记录 key 取 `uid`,没有则 `did`,
  再没有则 `c_ip` —— 同一主体的事件落到同一分区,下游按 key 分区计算时才不会被拆开。

  **这一项破了「采集器零外部依赖」,是明确的项目决定。** 引入 `franz-go`(连传递依赖
  共 5 个模块)。理由:零依赖是**手段**不是目的,而「让使用方自己接管道」的代价更高 ——
  管道断了采集器不知道,它的指标看不到管道那一头,风控链路上少一段数据不会报错,
  只会让策略不命中。自己实现协议子集能保住零依赖,但写错的表现往往是静默丢数据。

  SECURITY.md、采集器 README、Dockerfile、监控文档等 7 处关于零依赖的表述已如实修订,
  没有一处留着旧说法。

  异步生产 + 退出时 flush:`Write` 返回成功只表示进了发送队列,真正的投递失败在退出
  摘要里报并让进程非 0 退出。broker 连不上时**启动即失败** —— 常驻进程「起来了但一条
  也发不出去」是最难察觉的故障形态。`-kafka-acks=none` 会打警告。

- **CEL 的 `checkNotice`(策略级联)。** 查某个主体在过去一段时间内命中某条策略的次数,
  用于「先命中了 A,现在又出现 B」这类组合判定。这是 1.x SPL 里唯一真正可用的业务函数。

  两个引擎共用 `tests/golden/vectors/check-notice.json` 的 8 个场景对照。

  三条实现时定下、规格里补写的语义:

  - 时间窗 **[now − within×1000, now)**,**终点不含**。同一条事件里先求值的策略刚产出的
    告警不算 —— 否则结果会依赖**策略的求值顺序**,而顺序不是契约(策略是按文件名读进来的)。
    这条是写向量时发现的:第一版终点含,probe 在前置策略命中的**同一条事件**上就命中了。
  - **数的是已产出的告警**,被去重压掉的不算。去重意味着这条告警没被报出去,而级联问的是
    「之前报过没有」。
  - 告警历史索引的保留期取全部策略里最大的那个 `withinSeconds`,没有策略用它时索引完全
    不建。**不设上限就是内存泄漏。**

  `keyType` 对齐到领域模型:取 check_type(`IP` / `USER` / `DeviceID` / `OrderID`),
  保留 1.x 的 `ip` / `uid` / `did` 作为别名。**规格初稿里的 `page` 不接受** —— 名单模型
  里没有对应的 check_type,按它查永远查不到东西。未知取值抛错而不是静默降级。
...
- **文档里的测试数量连续两次发布都是错的。** v0.4.0 与 v0.5.0 的发布前复审都在
  `docs/guide/quickstart.md` 的「整个项目的测试分布」上栽了一次 —— 加了测试之后
  没人回来改文档,第一次靠人工核对修好,**第二次又错了**。

  加了 `tools/check_test_counts.py`,并入 `make validate` 与 CI。`--update` 可一键
  改文档。**派生数字应当由检查器盯着,不是由纪律盯着。**

  写它的时候自己也踩了一次同类问题:初版靠数源码里的 `test(` 调用点,数出 78 而实跑
  是 177 —— 差在向量测试从 JSON 循环生成用例。改成实跑读 TAP 输出。**派生数字要从
  事实来,不能从形态推断。**

- **隐私检查把「OpenResty 1.31.1.1」判成公网 IP。** 版本号误报的启发式此前只认「前面
  是斜杠」与「以 .0.0 结尾」,认不出「软件名 + 空格/括号 + 四段数字」。补了这一条,
  并加了**反向断言**:软件名与 IP 之间隔了别的词时(如「OpenResty 部署在 <某个公网 IP> 上」)
  仍然要拦 —— 放宽召回率是这类检查最危险的改动方向。

- **CI 编译采集器用的 Go 版本(1.22)与发布镜像(1.26)不一致** —— 也就是说 CI 测的
  和实际发布的不是同一个 Go。引入 franz-go(要求 1.25+)时暴露出来,已把 CI 对齐到
  Dockerfile 的版本。

- **`packages/cel-functions/README.md` 整体标着「🚧 实现中,尚无代码」。** 而
  `inTimeWindow` 与 `ipLocation` 两个引擎里**早就实现了** —— 内置资产里就有 3 条策略在用。
  改为逐函数的实现状态表。
- **同一份文档里写着「缺少任何一项 CI 都会失败」。** CI 里没有任何一项检查
  `packages/cel-functions/`。一条假的门禁承诺比没有门禁更危险 —— 它会让人以为有东西在把关。
- **`docs/guide/cel-reference.md` 里 `checkNotice` 的签名是两参数版**,与规格的四参数不符,
  且标着「未实现」。
...
[Unreleased]: https://github.com/threathunterX/nebula2/compare/v0.5.0...HEAD
[0.5.0]: https://github.com/threathunterX/nebula2/compare/v0.4.0...v0.5.0
